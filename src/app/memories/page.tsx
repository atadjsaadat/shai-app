                ...
                <div key={group.dateKey} className={styles.entryGroup}>
                  ...
                  {group.items.map(({ entry }) => (
                    <div
                      ...
                      onClick={() => { if (editingId !== entry.id) setSelectedEntryId(prev => prev === entry.id ? null : entry.id); }}
                    >
                      {editingId === entry.id ? (
                        <>
                        ...
                        <>
                          <div className={styles.entryHeader}>
                            ...
                            {selectedEntryId === entry.id && (
                              <div className={styles.entryControlsRow} onClick={e => e.stopPropagation()}>
                                <button className={styles.detailClose} onClick={() => { setSelectedEntryId(null); setDeleteEntryConfirm(null); }} aria-label="Close">
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                </button>
                                <button className={styles.detailDeleteBtn} onClick={() => setDeleteEntryConfirm(prev => prev === entry.id ? null : entry.id)} aria-label="Delete">
                                ...
                                <button className={styles.detailEditBtn} onClick={() => { startEdit(entry); setSelectedEntryId(null); setDeleteEntryConfirm(null); }}>
                          ...
                          {deleteEntryConfirm === entry.id && (
                            <div className={styles.deleteConfirmBanner} onClick={e => e.stopPropagation()}>
                              <p className={styles.deleteConfirmText}>Delete this entry?</p>
                              <div className={styles.deleteConfirmBtns}>
                                <button className={styles.deleteConfirmBtn} onClick={() => { deleteEntry(entry.id); setDeleteEntryConfirm(null); setSelectedEntryId(null); }}>Yes, delete</button>
                                <button className={styles.deleteCancelBtn} onClick={() => setDeleteEntryConfirm(null)}>Cancel</button>
                              </div>
                            </div>
                          )}
